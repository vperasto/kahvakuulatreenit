...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let remainingTime = 0;
    // HUOM: Muutettu TimerState takaisin yksinkertaisemmaksi, koska subStaten päättely oli vaikeaa
    const TimerState = { IDLE: 'idle', RUNNING: 'running', PAUSED: 'paused', FINISHED: 'finished' };
    let timerState = TimerState.IDLE;
    let pausedSubState = null; // Tallentaa onko pausetettu työstä ('work') vai levosta ('rest'/'round_rest')
    ...
    async function loadAppData() {
        ...
        try {
            ...
            // Tarkista kaikki tarvittavat osiot
            ...
            populateSingleRoutineButton('warmup'); // Käytä tätä funktiota
            populateSingleRoutineButton('cooldown'); // Käytä tätä funktiota
            resetAppState();
    ...
    function populateSingleRoutineButton(type) {
        ...
        button.dataset.routineType = type;
        button.addEventListener('click', () => handleRoutineSelect(type));
    ...
    function handleWeekSelect(weekNumber) {
         ...
         const mappedExercises = phaseExercisesList.map(phaseEx => {
              if (!phaseEx?.exerciseId) return null; const fullEx = fullData.exercises.find(ex => ex.id === phaseEx.exerciseId); if (!fEx) return null; return { ...fullEx, displayTitle: phaseEx.displayTitle || fEx.name, notes: phaseEx.notes || '', workTime, restTime };
         }).filter(ex => ex !== null);
         if (mappedExercises.length === 0) { console.error(`No valid exercises`); resetAppState(); itemNameH2.textContent = "Kelvollisia harjoituksia ei löytynyt."; return; }
    ...
    function handleRoutineSelect(type) {
        ...
        const routineData = fullData[type];
        if (!routineData || !routineData.steps) { console.error(`Routine data missing`); return; }
    ...
    function highlightRoutineButton(activeType = null) {
        ...
        containers.forEach(container => { container?.querySelectorAll('.routine-button')?.forEach(btn => { const isActive = btn.dataset.routineType === activeType; btn.classList.toggle('active', isActive); }); });
    ...
    function updateInfoAreaDisplay() {
        if (appMode === AppMode.WARMUP || appMode === AppMode.COOLDOWN) { infoAreaTitle.textContent = (appMode === AppMode.WARMUP ? "Lämmittely" : "Jäähdyttely"); infoAreaNotes.textContent = currentRoutine?.description || `Valittu ${appMode === AppMode.WARMUP ? 'lämmittely' : 'jäähdyttely'}.`; }
        else if (appMode === AppMode.WORKOUT && currentWorkoutInfo.week !== null) { infoAreaTitle.textContent = `Viikko ${currentWorkoutInfo.week} Tiedot`; const levelDesc = fullData?.kettlebellProgram11Weeks?.programInfo?.levels?.find(l => l.level == currentWorkoutInfo.level)?.description || ''; const focus = currentWorkoutInfo.focus ? `\nFokus: ${currentWorkoutInfo.focus}` : ''; infoAreaNotes.textContent = `Taso: ${currentWorkoutInfo.level} (${levelDesc})${focus}`; }
    ...
    function populateStepsList() {
        stepsListUl.innerHTML = ''; let items = []; let title = "Vaiheet / Harjoitukset";
        if (appMode === AppMode.WARMUP || appMode === AppMode.COOLDOWN) { items = currentRoutineSteps; title = (appMode === AppMode.WARMUP ? "Lämmittely" : "Jäähdyttely") + " Vaiheet"; }
        else if (appMode === AppMode.WORKOUT) { items = currentWorkoutExercises; title = currentWorkoutInfo.week ? `Viikko ${currentWorkoutInfo.week} Harjoitukset` : "Treenin Harjoitukset"; }
        stepsListTitle.textContent = title; if (items.length === 0) { stepsListUl.innerHTML = '<li>Valitse toiminto ensin.</li>'; return; }
    ...
    function displayItem(index) {
         let item = null; let type = 'step';
         if (appMode === AppMode.WORKOUT) { if (index < 0 || index >= currentWorkoutExercises.length) return; item = currentWorkoutExercises[index]; type = 'exercise'; }
         else if (appMode === AppMode.WARMUP || appMode === AppMode.COOLDOWN) { if (index < 0 || index >= currentRoutineSteps.length) return; item = currentRoutineSteps[index]; }
         else { itemNameH2.textContent = "Valitse toiminto"; itemDescriptionP.textContent = ""; itemImageImg.style.display = 'none'; return; }
         if (!item) return; itemNameH2.textContent = item.displayTitle || item.name; itemDescriptionP.textContent = type === 'exercise' ? `${item.description || ''}${item.notes ? `\n\nHuom: ${item.notes}` : ''}` : (item.description || '');
         if (type === 'exercise' && item.image) { itemImageImg.src = item.image; itemImageImg.alt = item.displayTitle || item.name; itemImageImg.style.display = 'block'; } else { itemImageImg.style.display = 'none'; itemImageImg.src = ''; itemImageImg.alt = ''; }
         if (type === 'exercise' && (timerState === TimerState.IDLE || timerState === TimerState.FINISHED)) { remainingTime = item.workTime || 0; updateTimerDisplay(remainingTime, "Työaika"); } // Aseta label heti
         else if (type === 'step') { updateTimerDisplay(0, "Seuraa ohjeita"); }
    ...
    function highlightCurrentStep() {
         const items = stepsListUl.querySelectorAll('li.step-item'); let currentIdx = -1; if(appMode === AppMode.WORKOUT) currentIdx = currentWorkoutIndex; else if(appMode === AppMode.WARMUP || appMode === AppMode.COOLDOWN) currentIdx = currentRoutineIndex; items.forEach((item) => { const idx = parseInt(item.dataset.index, 10); const isActive = !isNaN(idx) && idx === currentIdx; item.classList.toggle('active', isActive); if (isActive && (item.offsetTop < stepsListUl.scrollTop || item.offsetTop + item.offsetHeight > stepsListUl.scrollTop + stepsListUl.clientHeight)) { item.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } }); if (currentIdx === -1) { items.forEach(item => item.classList.remove('active')); }
    }

    // --- Ajastimen toiminnot ---
    function startPauseResumeWorkout() {
    ...
    function proceedWithStart() {
        console.log("Starting workout..."); currentWorkoutIndex = 0; currentWorkoutRound = 1; updateRoundDisplay();
        displayItem(currentWorkoutIndex); addBodyLock();
        // Aloitetaan suoraan työajalla
        startTimerPhaseInternal('work', currentWorkoutExercises[currentWorkoutIndex].workTime);
    }
    function pauseResumeTimerInternal() {
        if (appMode !== AppMode.WORKOUT) return;
        if (timerState === TimerState.RUNNING) {
            pausedSubState = determineSubStateFromLabel(); // Tallenna tarkempi tila
            stopTimerInterval(); timerState = TimerState.PAUSED; console.log("Paused");
            timerDiv.classList.add('timer-paused'); updateControlButtons(); updateTimerDisplay(remainingTime, "Tauko"); // Päivitä label heti
        } else if (timerState === TimerState.PAUSED) {
            console.log("Resumed"); timerState = TimerState.RUNNING;
            const wasResting = pausedSubState === 'rest' || pausedSubState === 'round_rest';
            pausedSubState = null; runTimerInterval(); timerDiv.classList.remove('timer-paused');
            if(wasResting){ timerDiv.classList.add('timer-resting'); highlightNextExercise(); }
    ...
    // Apufunktio päättelemään tarkempi tila labelista
    function determineSubStateFromLabel() {
         const label = timerLabelP.textContent.toLowerCase();
         if (label.includes("lepo")) return 'rest';
         if (label.includes("kierroslepo")) return 'round_rest';
         if (label.includes("työaika")) return 'work';
         return null; // Jos ei voida päätellä (esim. Tauko, Valmis)
    }
    function stopWorkoutInternal() {
        if (appMode !== AppMode.WORKOUT) return; stopTimer(); console.log("Stopped"); clearNextUpHighlight(); removeBodyLock();
        currentWorkoutRound = 1; pausedSubState = null;
        if (currentWorkoutExercises.length > 0 && currentWorkoutExercises[currentWorkoutIndex]) { const currentEx = currentWorkoutExercises[currentWorkoutIndex]; updateTimerDisplay(currentEx.workTime, "Työaika"); displayItem(currentWorkoutIndex); }
        else { resetAppState(); } updateRoundDisplay(); updateControlButtons();
    }
    ...
    // Yhdistetty funktio ajastinvaiheen käynnistämiseen
    function startTimerPhaseInternal(subState, duration) {
        stopTimerInterval(); timerState = TimerState.RUNNING; // Yksi yleinen running-tila
        remainingTime = duration;
        timerDiv.classList.remove('timer-resting', 'timer-paused'); clearNextUpHighlight();

        if (subState === 'work') {
            displayItem(currentWorkoutIndex); highlightCurrentStep();
            updateTimerDisplay(remainingTime, "Työaika");
        } else if (subState === 'rest') {
            timerDiv.classList.add('timer-resting');
            const nextIdx = currentWorkoutIndex + 1;
            if (nextIdx < currentWorkoutExercises.length) { displayItem(nextIdx); highlightNextExercise(); }
            else { displayItem(currentWorkoutIndex); highlightCurrentStep(); } // Turva
            updateTimerDisplay(remainingTime, "Lepo");
        } else if (subState === 'round_rest') {
             timerDiv.classList.add('timer-resting');
             if (currentWorkoutExercises.length > 0) { displayItem(0); highlightNextExercise(0); }
             updateTimerDisplay(remainingTime, "Kierroslepo");
        }
    ...
    function runTimerInterval() {
        ...
        timerInterval = setInterval(() => {
            ...
            const subState = determineSubStateFromLabel(); // Lue labelista

            if(isAudioUnlocked){
                 if (subState === 'work') { if (checkTime === 10 || (checkTime >= 1 && checkTime <= 5)) { playSound(beepSound); } }
                 else if (subState === 'rest' || subState === 'round_rest') { if (checkTime >= 1 && checkTime <= 3) { playSound(beepSound); } }
            }
    ...
    function handleTimerEnd() {
        ...
        const endedSubState = determineSubStateFromLabel(); // Mistä tultiin?

        if (endedSubState === 'work') {
            const currentEx = currentWorkoutExercises[currentWorkoutIndex]; if (!currentEx) { resetAppState(); return; }
            const isLastEx = currentWorkoutIndex === currentWorkoutExercises.length - 1;
            const isLastR = currentWorkoutRound >= currentWorkoutInfo.rounds;
            const restDur = currentEx.restTime || 0;
            if (isLastEx) {
                if (isLastR) { moveToNextPhase('finished'); } // Lopetus
                else { const roundRest = currentWorkoutInfo.restBetweenRounds || 0; if (roundRest > 0) { startTimerPhaseInternal('round_rest', roundRest); } else { moveToNextPhase('next_round_no_rest'); } }
            } else { if (restDur > 0) { startTimerPhaseInternal('rest', restDur); } else { moveToNextPhase('next_exercise_no_rest'); } }
        } else if (endedSubState === 'rest' || endedSubState === 'round_rest') {
            clearNextUpHighlight(); moveToNextPhase(endedSubState === 'round_rest' ? 'next_round' : 'next_exercise');
        }
    }

    function moveToNextPhase(trigger) { // trigger kertoo mistä tultiin
         if (appMode !== AppMode.WORKOUT) return;

         // Päivitä indeksi ja kierros triggerin perusteella
         if (trigger === 'next_round' || trigger === 'next_round_no_rest') {
             currentWorkoutRound++; currentExerciseIndex = 0;
         } else if (trigger === 'next_exercise') {
              currentExerciseIndex++;
         } else if (trigger === 'next_exercise_no_rest'){
             currentExerciseIndex++;
         } else if (trigger === 'finished') {
              // Treeni loppui
         } else {
              console.warn("Unknown trigger in moveToNextPhase:", trigger);
         }

         // Tarkista onko treeni valmis
         if (currentWorkoutRound > currentWorkoutInfo.rounds) {
              ...
              infoAreaNotes.textContent = `Kaikki ${currentWorkoutInfo.rounds} kierrosta tehty!`;
              if(isAudioUnlocked) playSound(beepSound);
              appMode = AppMode.IDLE; // Aseta IDLE
              updateControlButtons(); // Päivitä napit
         }
         // Jos treeni jatkuu:
         else if (currentExerciseIndex < currentWorkoutExercises.length) {
              updateRoundDisplay();
              const nextEx = currentWorkoutExercises[currentWorkoutIndex];
              // Aloita aina työajalla
              startTimerPhaseInternal('work', nextEx.workTime);
         } else {
              // Jos index meni yli (ei pitäisi tapahtua jos logiikka oikein)
              console.error("State error: Index out of bounds after move.");
              resetAppState();
         }
     }
    ...
    function startRoutine(type) {
         ...
         displayItem(currentRoutineIndex); updateControlButtons(); highlightRoutineButton(type);
    }
    ...
    function updateTimerDisplay(timeInSeconds, forceLabel = null) {
        ...
        if (forceLabel) { label = forceLabel; } // Käytä annettua labelia ensin
        else if (appMode === AppMode.WORKOUT) {
            const subState = determineSubStateFromLabel(); // Lue labelista PÄÄTTELEMISEEN
            if (timerState === TimerState.RUNNING) {
                 // Määritä label uudelleen subStaten tai logiikan perusteella
                 // Tässä kohtaa pitäisi tietää, mikä vaihe *on* käynnissä
                 // Käytetään väliaikaisesti labelin lukemista
                 if (subState === 'work') label = "Työaika";
                 else if (subState === 'rest') label = "Lepo";
                 else if (subState === 'round_rest') label = "Kierroslepo";
                 else label = "Käynnissä"; // Jos ei voida päätellä
    ...
    function highlightNextExercise(forceIndex = -1) { clearNextUpHighlight(); let nextIdx = -1; if (forceIndex !== -1) { nextIdx = forceIndex; } else { const subState = determineSubStateFromLabel(); if(subState === 'rest' || subState === 'round_rest') nextIdx = (subState === 'round_rest' ? 0 : currentWorkoutIndex + 1); } if (nextIdx >= 0 && nextIdx < currentWorkoutExercises.length) { const nextItem = stepsListUl.querySelector(`li[data-index="${nextIdx}"]`); if (nextItem) nextItem.classList.add('next-up'); } }
